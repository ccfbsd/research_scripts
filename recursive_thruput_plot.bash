...
generate_thruput_plot_if_leaf() {
    ...
    if ! find "${dir}" -mindepth 1 -maxdepth 1 -type d | grep -q .; then
        ...
        local flow1_id=$(echo "${flow1_summary}" | awk '{printf "%s\n", $1}' | cut -d: -f2)
        ...
        local flow2_id=$(echo "${flow2_summary}" | awk '{printf "%s\n", $1}' | cut -d: -f2)
        ...
        local cwnd_output="${dir}/all_cwnd_chart.pdf"
        local thruput_output="${dir}/all_throughput_chart.pdf"
        ...
        echo "generating gnuplot figures..."

        cwnd_title_str="${ver_str} ${flow1_stack} stack ${flow1_cc} congestion window ${bdp}"
        throughput_title_str="${ver_str} ${flow1_stack} stack ${flow1_cc} throughput ${bdp}"
