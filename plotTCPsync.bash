...
flow1_cwnd_stats="avgCWND: ${f1_avg_cwnd}, minCWND: ${f1_min_cwnd}, maxCWND: ${f1_max_cwnd} segments"
flow1_srtt_stats="avgSRTT: ${f1_avg_srtt}, minSRTT: ${f1_min_srtt}, maxSRTT: ${f1_max_srtt} µs"
...
flow2_cwnd_stats="avgCWND: ${f2_avg_cwnd}, minCWND: ${f2_min_cwnd}, maxCWND: ${f2_max_cwnd} segments"
flow2_srtt_stats="avgSRTT: ${f2_avg_srtt}, minSRTT: ${f2_min_srtt}, maxSRTT: ${f2_max_srtt} µs"

max_cwnd_global=$(echo "$f1_max_cwnd $f2_max_cwnd" | awk '{print ($1 > $2) ? $1 : $2}')
max_srtt_global=$(echo "$f1_max_srtt $f2_max_srtt" | awk '{print ($1 > $2) ? $1 : $2}')
...
max_thruput_timeline=$(awk 'BEGIN {max = 0} {if ($2 > max) max = $2} END {print max}'\
                     ${agg_mbps})
#echo "max_thruput_timeline: [$max_thruput_timeline]"
ymax_thruput=$(echo "${max_thruput_timeline} * 1.6" | bc)
ymax_cwnd=$(echo "${max_cwnd_global} * 1.3" | bc)
ymax_srtt=$(echo "${max_srtt_global} * 1.6" | bc)
#echo "ymax_cwnd: [${ymax_cwnd}], ymax_srtt: [${ymax_srtt}], ymax_thruput: [${ymax_thruput}]"
...
set output "all_cwnd_srtt_thruput.pdf"
set encoding utf8
...
plot "${snd1_cw}" using 1:2 title "socket1: ${flow1_cwnd_stats}" with linespoints ls 1,\
     "${snd2_cw}" using 1:2 title "socket2: ${flow2_cwnd_stats}" with linespoints ls 2

# Second plot srtt
set title "${srtt_title_str}"
set ylabel "srtt (microsecond)"
set yrange [0:${ymax_srtt}]
plot "${snd1_cw}" using 1:3 title "socket1: ${flow1_srtt_stats}" with linespoints ls 1,\
     "${snd2_cw}" using 1:3 title "socket2: ${flow2_srtt_stats}" with linespoints ls 2
